...
> 最近更新：2026-07-31（v1 迁移完成 + i18n 加固提交后）
...
| CORE-002 | core 基础 layouts | ac7add4 | 🟡 | 本地 14 layouts 全为 i18n override（67 项台账） |
| CORE-003 | core 基础 includes | ac7add4 | 🟡 | 同上 |
...
| CORE-014 | af-popover/tooltip 样式 | core 1.0.11 | ⏸️ | 本地 Bootstrap tooltips 保留（compat css 未加载） |
| CORE-015 | Tailwind-first + style contract | ac7add4 | 🟡 | P6-1 决策：不加载 tailwind（站点 100% Bootstrap 渲染，零冲突）；compat 窗口 v1.0–v1.2 |
...
| CV-005 | JSONResume + jekyll_get_json | v0.16 | ✅ | 本地 4 语言化保留 |
...
| SEARCH-003 | search-data 性能 | 1.0.3 | N/A | 本地生成器不同，规模小 |
| ICONS-001 | al_icons CDN+SRI | ac7add4 | 🟡 | 决策：保留 vendored webfonts（离线可用）；gem wrapper 未启用，无双份 |
| AN-001 | al_analytics（GA 等） | ac7add4; 1.0.x | ✅ | P5-7 gem wrapper；扁平键兼容（G-H0H008S793） |
| AN-002 | Simple Analytics / Cloudflare | 1.0.1/1.0.2 | ⏸️ | 能力随 gem 具备，开关关闭 |
...
| SEO-004 | tocbot 侧边 TOC | ac7add4; core 1.0.7 | ⏸️ | 保留 jekyll-toc + bootstrap-toc；重新评估条件：本地模板开始用 v1 样式时 |
| BUILD-001 | Gemfile v1 wiring | ac7add4 | ✅ | P2-1 |
| BUILD-002 | minify/terser/purgecss | v0.16→v1 | ✅ | 保留（terser git pin） |
| BUILD-003 | Docker 工具链 | v0.16→v1 | N/A | 本地无 Docker，不引入 |
| BUILD-004 | CI：unit-tests/visual-regression/upgrade-check | ac7add4 | ⏸️ | 未引入（本地 i18n-audit.yml 已加；可后补） |
| BUILD-005 | 6 个 integration_*.sh | ac7add4 | ⏸️ | 未引入（可后补） |
...
| tocbot 替换 jekyll-toc | 拒绝（暂） | 本地侧栏定制 | 本地模板采纳 v1 样式时 |
| scholar badge 启用 | ⚠️ 待决策 | 本地 bib 用 `gscholar={true}` 而模板查 `google_scholar_id`（迁移前既有无渲染） | 决策：补字段启用 或 删 `_data/citations.yml` |
...
> 待办提醒：P0-1 基线 URL manifest 目前仅存于 `/private/tmp/labpage-baseline-url-manifest.txt`（139 条），建议后续移入仓库（如 `tests/baseline/`），否则 runbook 的验证步骤缺少永久基准。
